...
//<-- Benson end 20080505
	...
	netdev->priv_flags = INT_MAIN;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
	pNetDevOps->ndo_open		= usb_rtusb_open;
	pNetDevOps->ndo_stop			= usb_rtusb_close;
	pNetDevOps->ndo_start_xmit	= usb_rtusb_sendpkt;
	pNetDevOps->ndo_do_ioctl		= rt73_ioctl;
	pNetDevOps->ndo_get_stats 	= rt73_get_ether_stats;
#else	
	netdev->open = usb_rtusb_open;
	netdev->stop = usb_rtusb_close;
	//netdev->priv = pAd;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
	net_dev = pAd->net_dev;
	struct net_device_ops *pNetDevOps = (struct net_device_ops *)net_dev->netdev_ops;
#endif
