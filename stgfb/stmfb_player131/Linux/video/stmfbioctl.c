/***********************************************************************
 *
 * File: stgfb/Linux/video/stmfbioctl.c
                           ...
                           STMFBIO_BLT_DATA      *pData)
{
  ...
  DPRINTK("src(%d,%d,%d,%d) -> dst(%d,%d,%d,%d)\n",srcrect.left,srcrect.top,srcrect.right,srcrect.bottom,
...
static int stmfbio_blt(struct stmfb_info* i, u_long arg)
{
  ...
  if (!i->pBlitter || !i->fbSurface.ulMemory)
    return -ENODEV;
  ...
  if (!copy_from_user(&bltData,(void*)arg,sizeof(bltData)))
  {
    ...
    switch(bltData.operation)
    {
      ...
      case BLT_OP_COPY:
      {
        stm_blitter_surface_t src = i->fbSurface;
...
int stmfb_ioctl(struct fb_info* fb, u_int cmd, u_long arg)
{
  ...
  switch (cmd)
  {
    ...
    case STMFBIO_SET_VAR_SCREENINFO_EX:
    {
       ...
       * mode is set.
       */
      if(tmp.activate == STMFBIO_ACTIVATE_IMMEDIATE && !i->current_videomode_valid)
