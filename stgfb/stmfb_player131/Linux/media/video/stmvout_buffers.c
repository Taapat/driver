/***********************************************************************
 *
 * File: stgfb/Linux/media/video/stmvout_buffers.c
...
static void bufferFinishedCallback(void                                  *pUser,
...
restart:
  ...
  if (!buf->physicalAddr)
  {
    ...
    if(partname_idx == 0
       && (buf->pDev->planeConfig->flags & STMCORE_PLANE_MEM_ANY) == STMCORE_PLANE_MEM_ANY)
    {
      partname_idx++;
...
#define PAGE64MB_SIZE  (1<<26)
  if ((buf->physicalAddr & ~(PAGE64MB_SIZE - 1)) != ((buf->physicalAddr + size) & ~(PAGE64MB_SIZE - 1)))
  {
    ...
    buf->physicalAddr = bpa2_alloc_pages (part, nPages, PAGE64MB_SIZE / PAGE_SIZE, GFP_KERNEL);
  ...
  if ((buf->physicalAddr & ~(PAGE64MB_SIZE - 1)) != ((buf->physicalAddr + size) & ~(PAGE64MB_SIZE - 1)))
  {
    /* can only happen, if somebody else requested bpa2 memory while we were
       busy here */
    printk ("%s: %8.lx + %lu (%lu pages) crosses a 64MB boundary again! Ignoring this time...\n",
            __FUNCTION__, buf->physicalAddr, size, nPages);
  }
...
int stmvout_allocate_mmap_buffers(stvout_device_t *pDev, struct v4l2_requestbuffers* req)
{
  ...
  debug_msg("%s: %d buffers of 0x%x bytes\n",__FUNCTION__,req->count,(unsigned int)bufLen);
  ...
  for (i = 0; i < req->count; ++i)
  {
    ...
    {
      debug_msg("%s: Allocated buffer %d at 0x%08lx.\n",__FUNCTION__, i, (unsigned long)pDev->streamBuffers[i].physicalAddr);
      ...
      pDev->streamBuffers[i].bufferHeader.src.ulClutBusAddress = pDev->streamBuffers[i].clut_phys;
...
#define PAGE64MB_SIZE  (1<<26)
  if ((device->userClut_phys & ~(PAGE64MB_SIZE - 1)) != ((device->userClut_phys + size) & ~(PAGE64MB_SIZE - 1)))
  {
    printk ("%s: %8.lx + %lu (%lu pages) crosses a 64MB boundary, retrying!\n",
    ...
    device->userClut_phys = bpa2_alloc_pages (part, nPages, PAGE64MB_SIZE / PAGE_SIZE, GFP_KERNEL);
...
int stmvout_deallocate_mmap_buffers(stvout_device_t  *pDev)
{
  ...
  for(i = 0; i < pDev->n_streamBuffers; i++)
  {
    ...
    if (pDev->streamBuffers[i].physicalAddr != 0 && (pDev->streamBuffers[i].mapCount > 0))
      return 0;
  }
...
int stmvout_queue_buffer(stvout_device_t *pDev, struct v4l2_buffer* pVidBuf)
{
  ...
  if (pVidBuf->memory == V4L2_MEMORY_USERPTR)
  {
	...
	pBuf->bufferHeader.src.ulVideoBufferAddr = phys;
	...
	pBuf->bufferHeader.src.ulClutBusAddress = pDev->userClut_phys + (CLUT_SIZE * i);

	debug_msg ("%s: Found the physical pointer 0x%.8lx\n", __FUNCTION__, phys);
	break;
      }
    }
  ...
  switch (pDev->bufferFormat.fmt.pix.pixelformat) {
    ...
    case V4L2_PIX_FMT_CLUT2:
      if (!pVidBuf->reserved)
	printk (KERN_WARNING "%s: no CLUT given for %s but we need one for LUT buffers!\n",
		__FUNCTION__,
  ...
  if (pDev->state.ckey.flags & SCKCF_ENABLE
    ...
    pBuf->bufferHeader.src.ColorKey = pDev->state.ckey;
    pBuf->bufferHeader.src.ColorKey.enable &= ~VCSCEF_ACTIVATE_BUFFER;
  } else
    pBuf->bufferHeader.src.ColorKey.flags = SCKCF_NONE;
  ...
  switch(pBuf->vidBuf.field)
  {
    case V4L2_FIELD_NONE:
      pBuf->bufferHeader.info.nfields = 1;
      pBuf->bufferHeader.src.ulFlags &= ~STM_PLANE_SRC_INTERLACED;
      break;

    case V4L2_FIELD_INTERLACED:
      pBuf->bufferHeader.info.nfields = 2;
      pBuf->bufferHeader.src.ulFlags |= STM_PLANE_SRC_INTERLACED;
      /*
       ...
       */
      if(standard & (STM_OUTPUT_STD_NTSC_M | STM_OUTPUT_STD_NTSC_J | STM_OUTPUT_STD_NTSC_443))
    ...
    case V4L2_FIELD_INTERLACED_BT:
      pBuf->bufferHeader.info.nfields = 2;
      pBuf->bufferHeader.src.ulFlags |= (STM_PLANE_SRC_INTERLACED | STM_PLANE_SRC_BOTTOM_FIELD_FIRST);
      break;
    default:
  ...
  if((pBuf->bufferHeader.src.ulFlags & STM_PLANE_SRC_INTERLACED) &&
     (pCurrentMode->ModeParams.ScanType           == SCAN_P)     &&
     (planeCaps.ulCaps & PLANE_CAPS_DEINTERLACE) == 0)
  {
    err_msg("%s: de-interlaced content on a progressive display not supported\n", __FUNCTION__);
    return -EINVAL;
  }

  pBuf->bufferHeader.info.presentationTime = ((TIME64)pVidBuf->timestamp.tv_sec * USEC_PER_SEC) +
  ...
  if(pVidBuf->flags & V4L2_BUF_FLAG_NON_PREMULTIPLIED_ALPHA)
  {
    ...
    pBuf->bufferHeader.src.ulFlags &= ~STM_PLANE_SRC_PREMULTIPLIED_ALPHA;
  }
  else
  {
    pBuf->vidBuf.flags             &= ~V4L2_BUF_FLAG_NON_PREMULTIPLIED_ALPHA;
  ...
  if(pVidBuf->flags & V4L2_BUF_FLAG_RESCALE_COLOUR_TO_VIDEO_RANGE)
  {
    ...
    pBuf->bufferHeader.dst.ulFlags |= STM_PLANE_DST_RESCALE_TO_VIDEO_RANGE;
  }
  else
  {
    pBuf->vidBuf.flags             &= ~V4L2_BUF_FLAG_RESCALE_COLOUR_TO_VIDEO_RANGE;
  ...
  if(pVidBuf->flags & V4L2_BUF_FLAG_GRAPHICS)
  {
    ...
    pBuf->bufferHeader.info.ulFlags |= STM_PLANE_PRESENTATION_GRAPHICS;
  }
  else
  {
    pBuf->vidBuf.flags              &= ~V4L2_BUF_FLAG_GRAPHICS;
  ...
  switch(fmt->fmt.pix.field)
  {
    ...
    case V4L2_FIELD_INTERLACED_BT:
    {
      if(pCurrentMode->ModeParams.ScanType == SCAN_P &&
         (planeCaps.ulCaps & PLANE_CAPS_DEINTERLACE) == 0)
      {
        err_msg("%s: de-interlaced content on a progressive display not supported on this plane",__FUNCTION__);
...
int stmvout_set_output_crop(stvout_device_t *pDev, struct v4l2_crop *pCrop)
{
  ...
  debug_msg("%s: (%ld,%ld,%ld,%ld)\n",__FUNCTION__,(long)pCrop->c.left,(long)pCrop->c.top,(long)pCrop->c.width,(long)pCrop->c.height);
 ...
 *
 */
static void delete_buffers_from_list (const struct list_head * const list)
{
