/***********************************************************************
 *
 * File: stgfb/Linux/video/hdmikthread.c
...

{
  ...
  /*
   * Find the maximum pixel repeat we can use for SD/ED modes and then
   * find the right mode line; this is so we always have the maximum audio
   * bandwidth available.
   */
  if((*mode)->ModeParams.OutputStandards & STM_OUTPUT_STD_SD_MASK)
  {
    if(hdmi->edid_info.cea_capabilities & STM_CEA_CAPS_PIXEL_QUAD_SD)
    {
      repeat = 4;
    ...
    {
      /*
       * We must select at least 2x for SD modes to meet TMDS timings.
       * If the TV doesn't list this in EDID then we will either fail to
       * match the mode or if we are not using strict EDID semantics
       * will start the mode on the output regardless.
       */
      repeat = 2;
      *tvStandard |= STM_OUTPUT_STD_TMDS_PIXELREP_2X;
    }
  }
  else if((*mode)->ModeParams.OutputStandards & STM_OUTPUT_STD_ED_MASK)
  {
    if(hdmi->edid_info.cea_capabilities & STM_CEA_CAPS_PIXEL_QUAD_ED)
    {
      repeat = 4;
      *tvStandard |= STM_OUTPUT_STD_TMDS_PIXELREP_4X;
    }
    else if(hdmi->edid_info.cea_capabilities & STM_CEA_CAPS_PIXEL_DOUBLE_ED)
    {
      repeat = 2;
      *tvStandard |= STM_OUTPUT_STD_TMDS_PIXELREP_2X;
    }
  ...
  if(repeat>1)
  {
    const stm_mode_line_t *tmp;
    tmp = stm_display_output_find_display_mode(hdmi->main_output,
    ...
    if(!tmp)
    {
      DPRINTK("Cannot find valid pixel repeated mode\n");
      return -EINVAL;
    }

    DPRINTK("Found pixel repeated mode %d @ %p\n",tmp->Mode,tmp);
    *mode = tmp;
  }
