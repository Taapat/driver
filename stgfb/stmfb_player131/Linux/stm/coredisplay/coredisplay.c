...
#endif
{
  ...
  if(platform_data->aux_output_id!=-1)
    sz += snprintf(&buf[sz], PAGE_SIZE - sz, "AUX: Output ID=%d\n", platform_data->aux_output_id);
  else
    sz += snprintf(&buf[sz], PAGE_SIZE - sz, "AUX: (NONE)\n");
