/***********************************************************************
 *
 * File: stgfb/Gamma/GammaMixer.cpp
 * Copyright (c) 2000, 2004, 2005, 2009 STMicroelectronics Limited.
 *
...
CGammaMixer::CGammaMixer(CDisplayDevice* pDev, ULONG ulRegOffset)
{
  DENTRY();
  m_pDev        = pDev;
...
void CGammaMixer::SetMixerForPlanes(stm_plane_id_t planeID, bool isEnabled, stm_mixer_activation_t act)
{
  ...
  if(isEnabled)
    m_planeMasks |= planeID;
  else
    m_planeMasks &= ~planeID;


  DEBUGF2(2,("CGammaMixer::SetMixerForPlanes m_planeMasks  = 0x%lx\n",m_planeMasks));

  ULONG tmp = ReadMixReg(MXn_CTL_REG_OFFSET) & 0xffff0000;
  WriteMixReg(MXn_CTL_REG_OFFSET, (tmp | (m_planeMasks & 0xffff)));
