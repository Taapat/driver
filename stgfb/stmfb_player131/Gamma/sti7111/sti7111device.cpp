/***********************************************************************
 *
 * File: stgfb/Gamma/sti7111/sti7111device.cpp
...
#include <STMCommon/stmtvoutteletext.h>

#include <Gamma/GDPBDispOutput.h>
#include <Gamma/sti7200/sti7200cursor.h>
...
bool CSTi7111Device::CreateInfrastructure(void)
{
  ...
  m_pDENC = new CSTi7111DENC(this, STi7111_DENC_BASE, m_pTeletext);
...
bool CSTi7111Device::CreatePlanes(void)
{
  ...
  cursor = new CSTi7200Cursor(ulCompositorBaseAddr+STi7111_CUR_OFFSET);
...
bool CSTi7111Device::CreateOutputs(void)
{
  ...
  if((pMainOutput = new CSTi7111MainOutput(this, m_pVTG1,m_pVTG2, m_pDENC, m_pMainMixer, m_pFSynthHD, m_pFSynthSD, m_pAWGAnalog)) == 0)
  {
  ...
  if((m_pOutputs[STi7111_OUTPUT_IDX_VDP1_MAIN] = new CSTi7111AuxOutput(this, m_pVTG2, m_pDENC, m_pAuxMixer, m_pFSynthSD, m_pAWGAnalog, pMainOutput)) == 0)
  {
...
bool CSTi7111Device::CreateGraphics(void)
{
  ...
  if(!m_graphicsAccelerators[STi7111_BLITTER_IDX_VDP0_MAIN]
     || !m_graphicsAccelerators[STi7111_BLITTER_IDX_VDP1_MAIN])
  {
    DERROR("failed to get two AQs\n");
    return false;
  }
