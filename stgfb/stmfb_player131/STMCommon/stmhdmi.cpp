/***********************************************************************
 *
 * File: stgfb/STMCommon/stmhdmi.cpp
                   ...
                   CSTmVTG                    *pVTG): COutput(pDev, (ULONG)pVTG)
{
  ...
  WriteHDMIReg(STM_HDMI_CFG, ReadHDMIReg(STM_HDMI_CFG) & STM_HDMI_CFG_CP_EN);
...
bool CSTmHDMI::Create(void)
{
   ...
   */
  WriteHDMIReg(STM_HDMI_AUDN, 6272);
...
bool CSTmHDMI::StartV29PHY(const stm_mode_line_t *const pModeLine, ULONG tvStandard)
{
  ...
  if(!m_bSinkSupportsDeepcolour && (m_ulDeepcolourGCP != 0))
  {
  ...
  if(m_pPHYConfig != 0)
  {
    ...
    while(!((m_pPHYConfig[i].minTMDS == 0) && (m_pPHYConfig[i].maxTMDS == 0)))
    {
      if((m_pPHYConfig[i].minTMDS <= tmdsck) && (m_pPHYConfig[i].maxTMDS >= tmdsck))
      {
        ...
        WriteHDMIReg(STM_HDMI_SRZ_CTRL, (m_pPHYConfig[i].config[3] & ~STM_HDMI_SRZ_CTRL_POWER_DOWN));
  ...
  WriteHDMIReg(STM_HDMI_SRZ_CTRL,  0);
