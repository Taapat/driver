/***********************************************************************
 *
 * File: stgfb/STMCommon/stmfdvo.cpp
...
/*
 * This represents a FlexDVO block, fed by a HDFormatter block. It remains to
 * be seen if we ever need the FlexDVO without the HDFormatter, but we will
 * refactor later if that is required.
 */
    ...
    case STM_CTRL_SIGNAL_RANGE:
    {
      ...
      switch(m_signalRange)
      {
        case STM_SIGNAL_FILTER_SAV_EAV:
          DEBUGF2(2,("CSTmFDVO::SetControl - Filter SAV/EAV\n"));
          val |= HDFMT_CFG_CLIP_SAV_EAV;
          break;
        case STM_SIGNAL_VIDEO_RANGE:
          DEBUGF2(2,("CSTmFDVO::SetControl - Clip input to video range\n"));
          val |= HDFMT_CFG_CLIP_EN;
          break;
        default:
          DEBUGF2(2,("CSTmFDVO::SetControl - Full signal range\n"));
          break;
      }
...
bool CSTmFDVO::SetOutputFormat(ULONG format,const stm_mode_line_t* pModeLine)
{
  ...
  if(pModeLine)
  {
    ...
    {
      /*
       * 8bit ITUR656 is not defined for ED or HD modes
       */
      if((pModeLine->ModeParams.OutputStandards & STM_OUTPUT_STD_ED_MASK != 0) ||
         (pModeLine->ModeParams.OutputStandards & STM_OUTPUT_STD_HD_MASK != 0))
        return false;
    }
    ...
    WriteFDVOReg(FDVO_DOF_CFG, (config->flags |
                                FDVO_DOF_EN   |
                                (m_bEnable422ChromaFilter?FDVO_DOF_EN_CHROMA_FILTER:0)));
  }
