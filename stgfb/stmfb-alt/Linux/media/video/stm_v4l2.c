/***********************************************************************\
 *
 * File: stmfb/Linux/media/video/stm_v4l2.c
...
stm_v4l2_register_driver (const struct stm_v4l2_driver * const driver)
{
  ...
  for (i = 0; i < STM_V4L2_MAX_DEVICES; ++i) {
    const struct stm_v4l2_driver_control_range * const this = &driver->control_range[i];
    int j;

    /* last has to be > than first */
    if (unlikely (this->first > this->last)) {
	...
	return -EINVAL;
      ...
      if(this->first != 0 && this->last != 0) {
        ...
        for (k = 0; k < STM_V4L2_MAX_DEVICES; ++k) {
          ...
          if (unlikely ((this->first >= other->first
                         && this->first <= other->last)
                        || (this->last >= other->first
                            && this->last <= other->last))) {
            printk ("%s: trying to register a region already occupied: this/other: "
                    "%.8x->%.8x vs. %.8x->%.8x\n", __FUNCTION__,
		   ...
		   void         *arg)
{
  ...
  /* if no handle, then we need to get one or there would be no handle for a
     specified type */
  if (!handle
      || (handle && type != -1 && !handle->v4l2type[type].driver)) {
    if (!handle)
      handle = alloc_handle ();
