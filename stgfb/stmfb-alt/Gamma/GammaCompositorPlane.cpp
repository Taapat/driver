/***********************************************************************
 *
 * File: stgfb/Gamma/GammaCompositorPlane.cpp
                                                       ...
                                                       GAMMA_QUEUE_BUFFER_INFO           &qbinfo) const
{
    if(!qbinfo.isSourceInterlaced  && qbinfo.isDisplayInterlaced)
    {
        ...
        else
        {
            ...
            bool convert_to_interlaced = (qbinfo.dst.height < (qbinfo.src.height*m_fixedpointONE/maxverticaldownscale));
            if(convert_to_interlaced)
            {
                DEBUGF2(3,("%s: converting source to interlaced for downscaling\n", __PRETTY_FUNCTION__));
     ...
     */
    qbinfo.dstFrameHeight = qbinfo.isDisplayInterlaced?qbinfo.dst.height*2:qbinfo.dst.height;

    /*
     * Now adjust the source coordinate system to take into account line skipping
     */
    qbinfo.src.y      /= qbinfo.line_step;
    qbinfo.src.height /= qbinfo.line_step;
     ...
     */
    qbinfo.maxYCoordinate = (qbinfo.src.y / m_fixedpointONE) + qbinfo.verticalFilterInputSamples - 1;
...
void CGammaCompositorPlane::CalculateVerticalScaling(GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
     ...
     * to twice the destination height (which is now in field lines).
     */
    qbinfo.verticalFilterOutputSamples = qbinfo.isUsingProgressive2InterlacedHW?(qbinfo.dst.height*2):qbinfo.dst.height;
    qbinfo.verticalFilterInputSamples  = qbinfo.isHWDeinterlacing?(qbinfo.src.height*2):qbinfo.src.height;
