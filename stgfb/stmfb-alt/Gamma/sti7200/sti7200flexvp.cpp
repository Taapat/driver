/***********************************************************************
 *
 * File: stgfb/Gamma/sti7200/sti7200flexvp.cpp
...
CSTi7200FlexVP::Create (void)
{
   ...
   */
  m_mainVideo = new CSTi7200MainVideoPipe (m_planeID,
                                           m_videoPlug,
                             ...
                             const void                 * const user)
{
  ...
  struct STi7xxxMainVideoPipeSetup  displayNodeMain = { dei_setup : { 0 } };
  ...
  if (xvp_can_handle)
    {
      ...
      struct XVP_QUEUE_BUFFER_INFO xqbi = {
        ...
        VHSRC_LUMA_HSRC  : displayNodeMain.dei_setup.LUMA_HSRC
      };
...
write_next_field:
  ...
  m_isEnabled = m_mainVideo->m_isEnabled;
