/***********************************************************************
 *
 * File: stgfb/Gamma/stb7100/stb7100analogueouts.cpp
...
CSTb7100MainOutput::CSTb7100MainOutput(CDisplayDevice    *pDev,
                                       ...
                                       stm_plane_id_t     sharedPlane): CGenericGammaOutput(pDev, pDENC, pVTG1, pMixer, pFSynth)
{
  m_pVTG2      = pVTG2;
  m_pAWGAnalog = pAWG;
  m_pDVO       = 0;
...
bool CSTb7100MainOutput::Start(const stm_mode_line_t *mode, ULONG tvStandard)
{
  ...
  if(mode->ModeParams.ScanType == SCAN_I)
  {
    ...
    WriteVOSReg(DHDO_BLANKL, (mode->ModeParams.FullVBIHeight / 2) - mode->TimingParams.VSyncPulseWidth);
  }
  else
  {
    WriteVOSReg(DHDO_ACTIVEL, mode->ModeParams.ActiveAreaHeight);
    WriteVOSReg(DHDO_BLANKL,  mode->ModeParams.FullVBIHeight - mode->TimingParams.VSyncPulseWidth);
  }
...
CSTb7100AuxOutput::CSTb7100AuxOutput(CDisplayDevice   *pDev,
                                     ...
                                     CGammaMixer      *pMixer,
                                     CSTmFSynth       *pFSynth,
                                     stm_plane_id_t    sharedPlane): CGenericGammaOutput(pDev, pDENC, pVTG, pMixer, pFSynth)
{
