/***********************************************************************
 *
 * File: stgfb/os21/tests/blitter_test/blitter_test.cpp
...
stm_plane_id_t          planeid;
  ...
  blitter_interrupt = interrupt_handle(OS21_INTERRUPT_BDISP0_AQ1);
  ...
  blitter_interrupt = interrupt_handle(OS21_INTERRUPT_BDISP_AQ);
#elif defined(CONFIG_STI7141)
  vsync_interrupt = interrupt_handle(OS21_INTERRUPT_IRQ_MAIN_VTG_0);
  blitter_interrupt = interrupt_handle(OS21_INTERRUPT_BDISP_AQ1_IRQP);
#else
  vsync_interrupt = 0;
#endif
...

  {
    ...
    if(pHDMI)
    {
      ...
      hdmi_interrupt = interrupt_handle(OS21_INTERRUPT_IRQ_HDMI_7);
#else
      hdmi_interrupt = interrupt_handle(OS21_INTERRUPT_HDMI);
#endif
      if(hdmi_interrupt)
      {
