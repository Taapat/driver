/***********************************************************************
 *
 * File: stgfb/Gamma/GDPBDispOutput.cpp
...
bool CGDPBDispOutput::Start(const stm_mode_line_t* pModeLine, ULONG tvStandard)
{
  ...
  m_pGDPRegisters = pHWPlane->GetGDPRegisters();
...
void CGDPBDispOutput::UpdateHW(void)
{
  ...
  /*
   * This is where we program the GDP registers
   */
  if(m_bHWEnabled)
  {
    ...
    g_pIOS->MemcpyToDMAArea(m_pGDPRegisters, OFFSETOF(GENERIC_GDP_LLU_NODE,GDPn_PML), &pml, sizeof(ULONG));
  ...
  else if(m_bBuffersValid && (!isDisplayInterlaced || !isTopFieldOnDisplay))
  {
    ...
    g_pIOS->MemcpyToDMAArea(m_pGDPRegisters, (ULONG)&((GENERIC_GDP_LLU_NODE *)0)->GDPn_PML, &pml, sizeof(ULONG));
    /*
  ...
  switch(ctrl)
  {
    case STM_CTRL_BRIGHTNESS:
    {
      ...
      if(m_pCurrentMode)
      {
        ...
        g_pIOS->MemcpyToDMAArea(m_pGDPRegisters, (ULONG)&((GENERIC_GDP_LLU_NODE *)0)->GDPn_AGC, &agc, sizeof(ULONG));
      }
      break;
    }
...
bool CGDPBDispOutput::SetGDPConfiguration(const stm_mode_line_t* pModeLine)
{
  ...
  GDPSetup.GDPn_NVN = ((struct GENERIC_GDP_LLU_NODE*)m_pGDPRegisters->pData)->GDPn_NVN;
  GDPSetup.GDPn_CTL = GDP_CTL_PREMULT_FORMAT | GDP_CTL_WAIT_NEXT_VSYNC;
  ...
  g_pIOS->MemcpyToDMAArea(m_pGDPRegisters,0,&GDPSetup,sizeof(GDPSetup));
