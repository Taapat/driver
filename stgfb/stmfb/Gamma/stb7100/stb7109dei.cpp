/***********************************************************************
 *
 * File: stgfb/Gamma/stb7100/stb7109dei.cpp
...
#  define FMDSW_Init(x,y,z)                ST_SOME_ERROR
#  define FMDSW_Terminate(x)               ST_SOME_ERROR
#  define FMDSW_CalculateHwThresh(x,y)     ST_SOME_ERROR
#  define FMDSW_Reset(x)                   ST_SOME_ERROR
#  define FMDSW_CalculateHwWindow(x,y,z)   ( { g_pIOS->ZeroMemory (z, sizeof (*(z))); } )
#  define FMDSW_SetSource(x,y)             ST_NO_ERROR
...
CSTb7109DEI::CSTb7109DEI(stm_plane_id_t   GDPid,
                         CGammaVideoPlug *videoPlug,
                         ULONG            deiBaseAddr): CGammaCompositorVideoPlane (GDPid,
                                                                                    videoPlug,
                                                                                    deiBaseAddr)
{
  DEBUGF2 (2, (FENTRY ", self: %p, planeID: %d\n",
               __PRETTY_FUNCTION__, this, GDPid));
  ...
  /*
   * These values are 7109 specific and may be overridden by subclasses
   */
...
bool
CSTb7109DEI::SetControl (stm_plane_ctrl_t control,
                         ULONG            value)
{
  bool retval;
...
bool
CSTb7109DEI::GetControl (stm_plane_ctrl_t  control,
                         ULONG            *value) const
{
  bool retval;
...
bool CSTb7109DEI::getQueueBufferInfo(const stm_display_buffer_t * const pFrame,
...
bool CSTb7109DEI::PrepareNodeForQueueing(const stm_display_buffer_t * const pFrame,
                                         const void                 * const user,
...
bool CSTb7109DEI::QueueNode(const stm_display_buffer_t    * const pFrame,
...
bool CSTb7109DEI::QueueBuffer(const stm_display_buffer_t * const pFrame,
                              const void                 * const user)
{
...
void CSTb7109DEI::UpdateHW(bool isDisplayInterlaced,
                           ...
                           const TIME64 &vsyncTime)
{
    ...
    if (m_currentNode.isValid)
    {
        ...
        if(ReadVideoReg(P2I_PXF_IT_STATUS) & P2I_STATUS_FIFO_EMPTY)
        {
          ...
          if(m_bReportPixelFIFOUnderflow)
          {
             ...
             * get out of it.
             */
            g_pIDebug->IDebugPrintf("CSTb7109DEI::UpdateHW(): Pixel FIFO underflow");
    ...
    if(PeekNextNodeFromDisplayList(vsyncTime, displayNode))
    {
        ...
        WriteCommonSetup((DEISetup*)displayNode.dma_area.pData);
        ...
        nextfieldowner = (DEISetup*)m_pendingNode.dma_area.pData;
...
DEIFieldSetup *
CSTb7109DEI::SelectFieldSetup (bool           isTopFieldOnDisplay,
...
void
CSTb7109DEI::NextDEIFieldSetup (DEISetup * const setup,
                                bool             isTopNode,
                                bool             streamChanged)
{
...
void
CSTb7109DEI::PreviousDEIFieldSetup (DEISetup * const owner,
                                    bool      isTopNode,
...
void
CSTb7109DEI::UpdateDEIStateMachine (const DEIFieldSetup * const fieldsetup,
                                    DEISetup            * const setup)
{
    ...
    if ((dei_ctl & DEI_CTL_BYPASS) == 0 && !fieldsetup->bFieldSeen)
    {
        ...
        if (m_previousNode.isValid
            && m_bUseFMD)
          {
            /* we're going to process the FMD/DEI statistics and also do the
               ...
               CSTb7109DEI::UpdateHW() updated it to be m_currentNode now) ,
            ...
            if (m_FMD_FieldInfo.bFilmMode)
              {
                m_FMD_Film = true;
                g_pIDebug->IDebugPrintf ("Film mode -> pat/idx: %.2x/%.2x/phase: %s\n",
                                         fmd_field_info.ucPattern,
                                         fmd_field_info.ucFieldIndex,
                                         fmd_field_info.bPhase ? "yes" : "no");
              }
            else if (m_FMD_Film)
              {
                m_FMD_Film = false;
        ...
        else
        {
             ...
             * that way it is ready to start agin if we switch it back on.
             */
            m_MotionState = DEI_MOTION_OFF;
        }
        ...
        /* Deinterlacing modes are determined by the input sequence type:
           ...
           - progressive sequences (for example movie broadcast in PAL/50
             Hz) are deinterlaced with the field merging algorihtm */
...
void
CSTb7109DEI::WriteFieldSetup (const DEIFieldSetup * const field)
{
    DEBUGF2 (4, ("%s\n", __PRETTY_FUNCTION__));
...
void
CSTb7109DEI::WriteCommonSetup (const DEISetup * const setup)
{
    DEBUGF2 (4, ("%s\n", __PRETTY_FUNCTION__));
 ...
 */
void
CSTb7109DEI::selectScalingFilters (DEISetup                      &displayNode,
                                   ...
                                   const GAMMA_QUEUE_BUFFER_INFO &qbinfo)
{
    ...
    ULONG VC1flag = (pFrame->src.ulFlags & ( STM_PLANE_SRC_VC1_POSTPROCESS_LUMA
        ...
        || m_HVSRCState.LumaVInc != qbinfo.vsrcinc))
    {
        ...
        DEBUGF2 (3, ("%s 'hfluma = 0x%p hfchroma = 0x%p'\n",
                     __PRETTY_FUNCTION__, displayNode.hfluma, displayNode.hfchroma));
        DEBUGF2 (3, ("%s 'vfluma = 0x%p vfchroma = 0x%p'\n",
                     __PRETTY_FUNCTION__, displayNode.vfluma, displayNode.vfchroma));
...
void
CSTb7109DEI::setupHorizontalScaling (DEISetup                      &displayNode,
...
void
CSTb7109DEI::calculateVerticalFilterSetup (DEIFieldSetup           &field,
                                     const GAMMA_QUEUE_BUFFER_INFO &qbinfo,
...
void
CSTb7109DEI::setupProgressiveVerticalScaling (DEISetup                      &displayNode,
                                              const GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
...
void
CSTb7109DEI::setupDeInterlacedVerticalScaling (DEISetup                      &displayNode,
                                               const GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
...
void
CSTb7109DEI::setupHWDeInterlacedVerticalScaling (DEISetup                      &displayNode,
                                                 const GAMMA_QUEUE_BUFFER_INFO &qbinfo)
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
...
void
CSTb7109DEI::allocateMotionBuffers (DEIFieldSetup &field,
...
void
CSTb7109DEI::setupInterlacedVerticalScaling (DEISetup                      &displayNode,
                                             const GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
 ...
 */
void
CSTb7109DEI::set420MBMemoryAddressing (DEISetup                      &displayNode,
                                       const stm_display_buffer_t    * const pFrame,
                                       const GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
...
void
CSTb7109DEI::set422RInterleavedMemoryAddressing (DEISetup                      &displayNode,
                                                 const stm_display_buffer_t    * const pFrame,
                                                 const GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
...
void
CSTb7109DEI::setPlanarMemoryAddressing (DEISetup                      &displayNode,
                                        const stm_display_buffer_t    * const pFrame,
                                        const GAMMA_QUEUE_BUFFER_INFO &qbinfo) const
{
    DEBUGF2 (3, ("%s\n", __PRETTY_FUNCTION__));
