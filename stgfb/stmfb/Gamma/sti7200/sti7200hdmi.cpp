/***********************************************************************
 *
 * File: stgfb/Gamma/sti7200/sti7200hdmi.cpp
 ...
 * For Cut1 only
 */
#define STM_HDMI_GPOUT_CUT1_DTS_EN       (STM_HDMI_GPOUT_DTS_FORMAT_EN|STM_HDMI_GPOUT_DTS_SPDIF_MODE_EN)
...
CSTi7200HDMI::CSTi7200HDMI(
    ...
    COutput                        *pMainOutput,
    CSTmSDVTG                      *pVTG): CSTmHDFormatterHDMI(pDev, STM_HDMI_HW_V_1_4, hdfRevision,
                                                               ulHDMIBase, ulHDFormatterBase,
                                                               pMainOutput, pVTG)
{
  ULONG val;
  ...
  m_ulPCMPOffset   = ulHDMIBase+STi7200_HDMI_PCMPLAYER_OFFSET;
  m_ulSPDIFOffset  = ulHDMIBase+STi7200_HDMI_SPDIF_OFFSET;
  ...
  if((val & AUD_FSYN_CFG_FS2_NSB) == 0)
  {
    ...
    WriteAUDReg(AUD_FSYNB2_SDIV, SDIV_48KHZ_256FS);
    WriteAUDReg(AUD_FSYNB2_MD,   MD_48KHZ_256FS);
    WriteAUDReg(AUD_FSYNB2_PE,   PE_48KHZ_256FS);
...
void CSTi7200HDMI::GetAudioHWState(stm_hdmi_audio_state_t *state)
{
  ULONG audctrl;
...
CSTi7200HDMICut1::CSTi7200HDMICut1(
    CDisplayDevice          *pDev,
    CSTi7200LocalMainOutput *pMainOutput,
    CSTmSDVTG               *pVTG): CSTi7200HDMI(pDev, STi7200C1_HDMI_BASE, STi7200C1_LOCAL_FORMATTER_BASE, STM_HDF_HDMI_REV1, pMainOutput, pVTG)
{
}


CSTi7200HDMICut1::~CSTi7200HDMICut1(void) {}


bool CSTi7200HDMICut1::SetInputSource(ULONG src)
{
  DENTRY();
  ...
  /*
   * Irritatingly on the 7200cut1, you have to switch two bits in different
   * registers to change the audio source, why???? And one of them isn't
   * documented in the programming manuals. The first is done by the
   * base class.
   *
   * This is not required and is harmfull on the cut2 because the bit effects
   * the non-HDMI SPDIF player instead.
   */
                ...
                STM_AV_SOURCE_8CH_I2S_INPUT))
  {
    ...
    {
      WriteAUDReg(AUD_HDMI_CTRL, AUD_HDMI_CTRL_VIDEO_SPDIF_NPCM);
      break;
    }
