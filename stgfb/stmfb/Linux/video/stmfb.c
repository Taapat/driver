/***********************************************************************
 *
 * File: stgfb/Linux/video/stmfb.c
...
static int stmfb_parse_module_parameters(struct stmfb_info *i, int display)
{
  ...
  if(i->aux_config.caps & STMFBIO_OUTPUT_CAPS_SDTV_ENCODING)
    i->aux_config.sdtv_encoding = i->default_sd_encoding;
    ...
    switch(*componentvideo)
    {
      ...
      case 'c':
                         ...
                         STM_OUTPUT_CAPS_SD_YPrPb_CVBS_YC))
        {
          ...
          if(i->aux_config.outputid != 0)
            i->aux_config.analogue_config = STMFBIO_OUTPUT_ANALOGUE_YC;
          else
...
#define _ALIGN_UP(addr,size)	(((addr)+((size)-1))&(~((size)-1)))
  ...
  if(i->aux_config.outputid != 0)
  {
    ...
    if(stmfb_set_output_configuration(&i->aux_config,i)<0)
    {
      printk(KERN_WARNING "fb%d aux output configuration is unsupported\n",display);
      ret = -EINVAL;
      goto failed_auxmem;
    }
  }
...
static void stmfb_cleanup_info(struct stmfb_info *i)
{
  ...
  if(i->pFBAuxOutput)
  {
    stm_display_output_release(i->pFBAuxOutput);
    i->pFBAuxOutput = NULL;
  }
...
static int stmfb_probe_get_outputs(struct stmfb_info *i,struct stmcore_display_pipeline_data *pd)
{
  ...
  i->main_config.outputid = 1;
  stmfb_initialise_output_config(i->pFBMainOutput,&i->main_config);

  if(pd->aux_output_id != -1)
  {
    i->pFBAuxOutput = stm_display_get_output(pd->device, pd->aux_output_id);
    i->aux_config.outputid = 2;
    stmfb_initialise_output_config(i->pFBAuxOutput,&i->aux_config);
  }
...
static int stmfb_suspend(struct platform_device *pdev, pm_message_t state)
{
  ...
  if(i->pFBAuxOutput)
    stm_display_output_suspend(i->pFBAuxOutput);
...
static int stmfb_resume(struct platform_device *pdev)
{
  ...
  if(i->pFBAuxOutput)
    stm_display_output_resume(i->pFBAuxOutput);
