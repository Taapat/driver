...
#include "Linux/media/video/stmvout.h"
...
#define ltv_message(format, args...) \
  ( { printf (BLUECOL "(%.5d) message: " __FILE__ ": " format NOCOL "\n", getpid (), ## args); } )
#define ltv_warning(format, args...) \
  ( { printf (REDCOL "(%.5d) warning: " __FILE__ ": " format NOCOL "\n", getpid (), ## args); } )
      ...
      char *argv[])
{
  ...
  int do_list = 0,
      ...
      bcsh_plane_mode = 0
    ;
  while ((opt = getopt (argc, argv, "Cc:Dd:Xx:Nn:Tt:Ff:Ii:Rr:Kk:Vv:Bb:Zz:Mm:2lh")) != -1)
    {
      switch (opt)
        {
        ...
        default:
          fprintf (stderr,
                   ...
                   "\n"
                   "  -c CONFIG         enable (1)/disable (0) IQI\n"
                   "  -C                query currently set IQI mode\n"
                   "\n"
                   "  -d DEMO           enable (1)/disable (0) IQI demo mode\n"
                   "  -D                query IQI demo mode\n"
                   "\n"
                   "  -x MODE           set xVP mode: 0 bypass, 1: FilmGrain, 2: TNR, 3: TNR(bypass), 4: TNR(manual NLE)\n"
                   "  -X                query currently set xVP mode\n"
                   "\n"
                   "  -n value          value to use for NLE in when TNR mode is manual NLE\n"
                   "  -N                query currently set NLE override value\n"
                   "\n"
                   "  -t [0|1]          swap top/bottom bit in TNR algo\n"
                   "  -T                query currently set swap status\n"
                   "\n"
                   "  -f [0|1]          enable (1)/disable (0) FMD\n"
                   "  -F                query currently set FMD mode\n"
                   "\n"
                   "  -i [0|1|2]        set DEI mode: 0: 3D, 1: bypass, 2: median\n"
                   "  -I                query currently set DEI mode\n"
                   "\n"
                   "  -r value          set DEI ctrl register\n"
                   "  -R                query currently set DEI ctrl register\n"
                   "\n"
                   "  -k value          set color key\n"
                   "  -K                query currently set color key\n"
                   "\n"
                   "  -v value          Brightess\n"
                   "  -V                query currently set Brightess\n"
                   "\n"
                   "  -b value          Contrast\n"
                   "  -B                query currently set Contrast\n"
                   "\n"
                   "  -z value          Saturation\n"
                   "  -Z                query currently set Saturation\n"
                   "\n"
                   "  -m value          Hue\n"
                   "  -M                query currently set Hue\n"
                   "\n"
                   "  -2                let Brightness/Contrast/Sat/Hue work on plane \n"
                   "\n",
                   argv[0]);
          exit (EXIT_FAILURE);
          break;
