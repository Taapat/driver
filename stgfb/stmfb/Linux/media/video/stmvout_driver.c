	       ...
	       void                       *arg)
{
  ...
  case VIDIOC_REQBUFS:
	...
	goto err_busy;
      ...
      if (req->memory == V4L2_MEMORY_MMAP && !stmvout_allocate_mmap_buffers (pDev, req)) {
  ...
  case VIDIOC_STREAMOFF:
    ...
    /* FIXME: how to handle enum STM_VOUT_OUTPUT_ID
       -> pDev->displayPipe.aux_output_id == -1; */
  case VIDIOC_G_OUTPUT:
    {
      int i;
  ...
  case VIDIOC_ENUMOUTPUT:
	...
	goto err_inval;
      ...
      if ((ret = stmvout_get_supported_standards (&g_voutData[minor][index], &output->std)) < 0)
        goto err_ret;
    }
    break;
  ...
  case VIDIOC_QUERYCTRL:
    ret = stmvout_queryctrl (pDev, g_voutData[minor], (struct v4l2_queryctrl * ) arg);
    goto err_ret;
    break;

  case VIDIOC_QUERYMENU:
    ret = stmvout_querymenu (pDev, g_voutData[minor], (struct v4l2_querymenu *) arg);
    goto err_ret;
    break;

  case VIDIOC_S_CTRL:
    ret = stmvout_s_ctrl (pDev, g_voutData[minor], (struct v4l2_control *) arg);
    goto err_ret;
    break;

  case VIDIOC_G_CTRL:
    ret = stmvout_g_ctrl (pDev, g_voutData[minor], (struct v4l2_control *) arg);
    goto err_ret;
    break;
		     ...
		     int              planenr)
{
  ...
  i = 0;
  while((i < ARRAY_SIZE(device_names)) && (device_names[i].plane_id != pDev->planeConfig->id))
    i++;
