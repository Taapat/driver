...
///
//FrameParserStatus_t FrameParser_AudioWma_c::ParseFrameHeader( unsigned char *FrameHeaderBytes, WmaAudioParsedFrameHeader_t *ParsedFrameHeader )
//{
//    return FrameParserNoError;
//}

//{{{  ReadStreamParameters
FrameParserStatus_t FrameParser_AudioWma_c::ReadStreamParameters       (unsigned char*                  FrameHeaderBytes,
...
FrameParserStatus_t   FrameParser_AudioWma_c::ReadHeaders( void )
{
    ...
    if( 0 == memcmp(BufferData, asf_guid_lookup[ASF_GUID_STREAM_PROPERTIES_OBJECT], sizeof(asf_guid_t)))
    {
        ...
        Status = ReadStreamParameters (BufferData, StreamParameters);
        if (Status != FrameParserNoError)
        {
...
//?? somehow lost this one, but are the parameters likely to be valid and relate to coded frames?
//?? NO! NumberOfSamples is ALWAYS 0... :(
//    report( severity_info, "POSTDECODE: GenNxtFrmNormPbTm - SamplesPerBlock %d Frq %d\n",ParsedFrameHeader.SamplesPerBlock,
//                                             ParsedFrameHeader.SamplingFrequency);

//    GenerateNextFrameNormalizedPlaybackTime(ParsedFrameHeader.SamplesPerBlock,
//                                             ParsedFrameHeader.SamplingFrequency);
