...
{       
	...
	Configuration.CodedMemorySize                       = 0x800000;
	Configuration.MaximumCodedFrameSize                 = 0x100000;
	...
	Configuration.TrickModeParameters.MaximumNormalDecodeRate           = 2;
	...
	Configuration.TrickModeParameters.PixelsAtNormalRate            = 720 * 576;
	Configuration.TrickModeParameters.FrameRateAtNormalRate         = 25;
...
CodecStatus_t Codec_MmeVideoDivx_c::SendMMEStreamParameters( void )
{
	...
	if ( (CurrentVersion > 100) && DivxFirmware)
	{
	    ...
	    report(severity_info,"Using DivX firmware for version %d stream (codec %d)\n",CurrentVersion,TransformerID);
	}
	else
	{
	    TransformerID = SelectedTransformer;
	    report(severity_info,"Using MPEG4 P2 firmware for version %d stream (codec %d)\n",CurrentVersion,TransformerID);        
	    CurrentVersion = 500;
	}            
...
#ifndef __TDT__
    if ( (Parsed->VolHeader.width > 960) ||
	     (Parsed->VolHeader.height > 576) )
#else
