...
{       
	...
	Configuration.TransformName[0]                      = "MPEG4P2HD_TRANSFORMER0";
	Configuration.TransformName[1]                      = "MPEG4P2HD_TRANSFORMER1";
//	Configuration.TransformName[1]                      = DIVXDEC_MME_TRANSFORMER_NAME "1";
	...
	Configuration.TrickModeParameters.PixelsAtNormalRate            = 1280 * 720;
...
//	report(severity_error,"%s :: %d\n",__FUNCTION__,__LINE__);
	...
	if ((Parsed->VolHeader.version != 500 ) || Parsed->VolHeader.interlaced)
	{	
...
//	if ( (Parsed->VolHeader.width != StreamWidth) ||
	if ( (Parsed->VolHeader.width > 1280) ||
	     (Parsed->VolHeader.height > 720) )
	{
		report(severity_error,"We need to restart the transformer - to be implemented ...\n");
		Player->MarkStreamUnPlayable( Stream );
		return CodecError;
	}
