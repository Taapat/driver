...
//
    ...
    if (isFwEac3Capable)
    {
        ...
        // bit 0-1: upsampling performed if required
        // bit 2: endianess of the transcoded output (0: be)
        // bit 3: dither (must be set to 1 when transcoding)
		Config.Config[DDP_OUTPUT_SETTING] = 0x2 | (ACC_MME_TRUE << 2) |  (ACC_MME_TRUE << 3); // upsampling is never done, transcoded output is le
    }
...
#if __KERNEL__
        AStatus = PartitionAllocatorOpen( &TranscodedFrameMemoryDevice, Configuration.CodedMemoryPartitionName, EAC3_FRAME_MAX_SIZE * EAC3_TRANSCODE_BUFFER_COUNT, true );
