...
PlayerStatus_t Mixer_Mme_c::UpdatePlayerComponentsModuleParameters()
{
    ...
    CodecParameterBlock_t       ParameterBlockC;
    ...
    ParameterBlockC.ParameterType = CodecSpecifyDRC;
    ParameterBlockC.DRC.Enable    = OutputConfiguration.drc_enable; // ON/OFF switch of DRC 
    ParameterBlockC.DRC.Type      = OutputConfiguration.drc_type;   // Line/RF/Custom mode
    ParameterBlockC.DRC.HDR       = OutputConfiguration.hdr;        // Boost factor
    ParameterBlockC.DRC.LDR       = OutputConfiguration.ldr;        // Cut   factor
    MIXER_DEBUG( "Master A/V  DRC is {Enable = %d / Type = %d / HDR = %d / LDR = %d} \n",
	         ParameterBlockC.DRC.Enable, ParameterBlockC.DRC.Type,
	         ParameterBlockC.DRC.HDR,  ParameterBlockC.DRC.LDR );
	...
	if( Clients[i].State != DISCONNECTED && Clients[i].State != STOPPED && Clients[i].State != STOPPING )
	{
	    ...
	    Status = Clients[i].Manifestor->SetModuleParameters( sizeof(ParameterBlockC),
		                                                 (void *) &ParameterBlockC );
	    if ( ManifestorNoError != Status )
	    {
		Result = PlayerError;
		break;
...
//
    ...
    if( (ActualTopology.card[0].flags & SND_PSEUDO_TOPOLOGY_FLAGS_FATPIPE) &&
	OutputConfiguration.spdif_encoding == SND_PSEUDO_MIXER_SPDIF_ENCODING_FATPIPE )
    {
        // FatPipe is magical...
        MixerParams.OutputNbChannels = 8;
    }
    else
    {
	...
	MME_LxPcmPostProcessingGlobalParameters_Frozen_t & PcmParams,  int dev_num, bool EnableDMix )
{
    ...
    bool TraceDownmixLookups = OutputMode != LastOutputMode[dev_num] ||
	                       InputMode != LastOutputMode[dev_num];
    ...
    if (OutputMode != ACC_MODE_ID)
        OutputId.ca = TranslateAudioModeToChannelAssignment(OutputMode);
    else
        OutputId.n = 0xffffffff; // magic wildcard value
    InputId.ca = TranslateAudioModeToChannelAssignment(InputMode);
    uint64_t TargetSortValue = ((uint64_t) OutputId.n << 32) + InputId.n;

    // single comparision (i.e. fixed time) binary search
    unsigned int low = 0;
    unsigned int high = DownmixFirmware->header.num_index_entries;
    uint64_t CurrentSortValue;
	...
	CurrentSortValue = ((uint64_t) OutputId.n << 32) + InputId.n;
    ...
    if( CurrentSortValue == TargetSortValue )
    {
	struct snd_pseudo_mixer_downmix_index *index = DownmixFirmware->index + low;
...
//
    ...
    else
    {
        ...
        else
        {
            ...
            // best.usin    g _ENABLE causes the processing to unconditionally applied at the start of the chain.   
    ...
    MIXER_DEBUG( "Enabled post-mix resampling (%s)\n            ",
...
//
    ...
    {
    case PcmPlayer_c::OUTPUT_FATPIPE:
	CMC.Config[CMC_OUTMODE_MAIN] = ACC_MODE_ID; /* FatPipe is a malleable output surface */
...
#define XXX(mode, p0, p1, p2, p3) { mode, #mode, { SND_PSEUDO_MIXER_CHANNEL_PAIR_ ## p0, \
  ...
  XXX( ACC_MODE10,	    	NOT_CONNECTED,	CNTR_0,		NOT_CONNECTED,	NOT_CONNECTED	), // Mad
    ...
    // CEA-861 modes (in numerical order)
...
//XXX( ACC_MODE40,	    	L_R,		NOT_CONNECTED,	NOT_CONNECTED,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE40_LFE,	    	L_R,		0_LFE1,		NOT_CONNECTED,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE50,	    	L_R,		CNTR_0,		NOT_CONNECTED,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE50_LFE,	    	L_R,		CNTR_LFE1,	NOT_CONNECTED,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE41,	    	L_R,		NOT_CONNECTED,	CSURR_0,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE41_LFE,	    	L_R,		0_LFE1,		CSURR_0,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE51,	    	L_R,		CNTR_0,		CSURR_0,	CNTRL_CNTRR	), // No enum
//XXX( ACC_MODE51_LFE,	    	L_R,		CNTR_LFE1,	CSURR_0,	CNTRL_CNTRR	), // No enum
  XXX( ACC_MODE42,	    	L_R,		NOT_CONNECTED,	LSUR_RSUR,	CNTRL_CNTRR	), // Bug 4518
//XXX( ACC_MODE42_LFE,	    	L_R,		0_LFE1,		LSUR_RSUR,	CNTRL_CNTRR	), // No enum
  XXX( ACC_MODE52,	    	L_R,		CNTR_0,		LSUR_RSUR,	CNTRL_CNTRR	), // Bug 4518
//XXX( ACC_MODE52_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	CNTRL_CNTRR	), // No enum

    // DTS-HD speaker topologies.
    ...
    // (by disconnecting pair3) if the use requests any of these modes.
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	LSIDESURR_RSIDESURR ),
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	LHIGHSIDE_RHIGHSIDE ),
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	CSURR_TOPSUR	),
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	LHIGH_RHIGH	),
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	LWIDE_RWIDE	),
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	CSURR_CHIGH	),
...
enum eAccAcMode Mixer_Mme_c::TranslateChannelAssignmentToAudioMode( struct snd_pseudo_mixer_channel_assignment ChannelAssignment )
{
    ...
    // both of these should already be the case but just in case...
