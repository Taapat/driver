...
struct cx21143_state {
	struct dvb_frontend_ops 		ops;
	struct dvb_frontend 			frontend;

	const struct cx21143_config* 		config;

	struct cx21143_tuning 			dcur;
	struct cx21143_tuning 			dnxt;

	struct semaphore			fw_load_sem;
/* FIXME: remove thread_id if not using loader thread */
	int					thread_id;

	struct cx21143_cmd 			dsec_cmd;
