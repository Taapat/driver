...
static int core_get_frequency(struct dvb_frontend *fe, u32 *frequency)
{
...
/* copied from budget-av */
static int core_set_frequency(struct dvb_frontend *fe, u32 frequency)
{
	struct dvb_frontend_ops	*frontend_ops = NULL;
	struct dvb_tuner_ops	*tuner_ops = NULL;
	struct tuner_state	t_state;
...
/* copied from budget-av */
static int core_get_bandwidth(struct dvb_frontend *fe, u32 *bandwidth)
{
...
static int core_set_bandwidth(struct dvb_frontend *fe, u32 bandwidth)
{
	struct dvb_frontend_ops	*frontend_ops = NULL;
	struct dvb_tuner_ops	*tuner_ops = NULL;
	struct tuner_state	t_state;
...
static struct stb0899_config stb0899_config = {
	...
	.tuner_get_frequency	= core_get_frequency,
	.tuner_set_frequency	= core_set_frequency,
	.tuner_set_bandwidth	= core_set_bandwidth,
	.tuner_get_bandwidth	= core_get_bandwidth,
                     ...
                     struct plat_tuner_config *tuner_cfg, int i)
{
  ...
  if ((cfg->i2c_adap == NULL) || (cfg->tuner_reset_pin == NULL) ||
    ...
    printk ("stb0899: failed to allocate resources (%s)\n",
...
void fe_core_register_frontend(struct dvb_adapter *dvb_adap)
{
	...
	printk (KERN_INFO "%s: DVB: Spider-Team STB0899 frontend core\n", __FUNCTION__);
