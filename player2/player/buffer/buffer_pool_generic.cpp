...
//
    ...
    for( i=0; i<MAX_ATTACHED_BUFFERS; i++ )
	if( LocalBuffer->AttachedBuffers[i] != NULL )
	{
	    LocalBuffer->AttachedBuffers[i]->DecrementReferenceCount();
	    LocalBuffer->AttachedBuffers[i]     = NULL;
	}
...
//
	...
	{
	    CountNonZero++;
	    Allocated   += Buffer->BufferBlock->Size;
	    SizeInUse   += Buffer->DataSize;
	}
    }
...
//
    ...
    TotalMemory         = 0;
    if( BufferDescriptor->AllocationSource == AllocateFromSuppliedBlock )
	TotalMemory     = Size;
    else if( BufferDescriptor->AllocationSource == AllocateIndividualSuppliedBlocks )
...
//
	...
	*BuffersWithNonZeroReferenceCount       = CountNonZero;
...
BufferStatus_t   BufferPool_Generic_c::ExpandMemoryBlock( 
						BlockDescriptor_t         Block,
						unsigned int              NewSize )
{
BufferStatus_t            Status;
BufferDataDescriptor_t   *Descriptor;
struct BlockDescriptor_s  NewBlockStructure;
BlockDescriptor_t         NewBlock;
AddressType_t             SourceAddressType;
AddressType_t             DestinationAddressType;

//

    NewBlock            = &NewBlockStructure;
    Descriptor          = Block->Descriptor;
    ...
    //
    // Check that Expandage is appropriate.
    //

    if( NewSize <= Block->Size )
    {
	report( severity_error, "BufferPool_Generic_c::ExpandMemoryBlock - Attempt to expand a buffer to the same, or a smaller size.\n\t\tThis must be some new usage of the word 'Expand' that I was previously unaware of.\n" );
	return BufferError;
    }
	...
	(Block->Address[UnCachedAddress] == NULL) )
    {
	report( severity_error, "BufferPool_Generic_c::ExpandMemoryBlock - Unable to expand a block with only physical addresses (cannot copy the data).\n" );
	return BufferError;
    }


    //
    // Only specific allocation types can be expanded.
    //
	...
	case AllocateFromSuppliedBlock:
			...
			NewBlock->Next                  = NULL;
			NewBlock->Descriptor            = Descriptor;
			NewBlock->AttachedToPool        = true;
			NewBlock->Size                  = NewSize;
			NewBlock->MemoryAllocatorDevice = ALLOCATOR_INVALID_DEVICE;

			Status      = AllocateMemoryBlock( NewBlock, false, 0, MemoryPoolAllocator, MemoryPool, NULL, MemoryPartitionName, 
							   "BufferPool_Generic_c::ExpandMemoryBlock" );

			if( Status != BufferNoError )
			    return Status;

			//
			// Copy the buffer content into it
			//

			SourceAddressType               = (Block->Address[CachedAddress] != NULL) ? CachedAddress : UnCachedAddress;
			DestinationAddressType          = (NewBlock->Address[CachedAddress] != NULL) ? CachedAddress : UnCachedAddress;
			memcpy( NewBlock->Address[DestinationAddressType], Block->Address[SourceAddressType], Block->Size );
			...
			//
			// Copy new block descriptor into the buffer's block descriptor
			//

			memcpy( Block, NewBlock, sizeof(struct BlockDescriptor_s) );
			break;
    }
