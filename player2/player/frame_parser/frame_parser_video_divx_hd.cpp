/************************************************************************
COPYRIGHT (C) ST Microelectronics 2008
...
{
	Configuration.FrameParserName               = "Video Divx HD";
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::Reset(   void )
{
	FrameParameters = NULL; 
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::RegisterOutputBufferRing(
					Ring_t          Ring )
{
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::ForPlayProcessFrame(      void )
{
//      report ( severity_error, "%s : %d\n",__FUNCTION__,__LINE__);
...
//
/*
FrameParserStatus_t   FrameParser_VideoDivxHd_c::RevPlayProcessFrame(      void )
{
	//report ( severity_error, "%s : %d\n",__FUNCTION__,__LINE__);
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::ForPlayQueueFrameForDecode( void )
{
...
//
/*
FrameParserStatus_t   FrameParser_VideoDivxHd_c::RevPlayQueueFrameForDecode( void )
{
	//report ( severity_error, "%s : %d\n",__FUNCTION__,__LINE__);
...
//
/*
FrameParserStatus_t   FrameParser_VideoDivxHd_c::RevPlayProcessDecodeStacks(              void )
{
...
//
/*
FrameParserStatus_t   FrameParser_VideoDivxHd_c::RevPlayPurgeDecodeStacks(                void )
{
	//report ( severity_error, "%s : %d\n",__FUNCTION__,__LINE__);
...
/*                              int n = ParsedFrameParameters->DataOffset;
				report (severity_error, "%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n",
					BufferData[n],BufferData[n+1],BufferData[n+2],BufferData[n+3],BufferData[n+4],
					BufferData[n+5],BufferData[n+6],BufferData[n+7],BufferData[n+8],BufferData[n+9],
					BufferData[n+10],BufferData[n+11],BufferData[n+12],BufferData[n+13],BufferData[n+14],
					BufferData[n+15],BufferData[n+16],BufferData[n+17],BufferData[n+18],BufferData[n+19]);
*/                              
...
//                      report (severity_error,"%x END_OF_START_CODE_LIST\n",END_OF_START_CODE_LIST);
		...
		{
			report( severity_error, "FrameParser_VideoDivxHd_c::ReadHeaders - Unknown/Unsupported header 0x%02x\n", Code );
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::CommitFrameForDecode(void)
{
	...
	if (StreamParameters->VolHeader.aspect_ratio_info <= PAR_16_9_NTSC)             ParsedVideoParameters->Content.PixelAspectRatio = 
...
FrameParserStatus_t  FrameParser_VideoDivxHd_c::ReadVoHeader(void)
{	
...
FrameParserStatus_t  FrameParser_VideoDivxHd_c::ReadVolHeader( Mpeg4VolHeader_t       *Vol )
{
int i;
...
//
unsigned int FrameParser_VideoDivxHd_c::ReadVosHeader( )
{
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::PrepareReferenceFrameList( void )
{
unsigned int    i;
...
FrameParserStatus_t   FrameParser_VideoDivxHd_c::ForPlayUpdateReferenceFrameList( void )
{
