...
/*
				if (Vop.vop_coded == 0)
				{
                    report(severity_info, "Found NULL Frame\n");
				} else report(severity_info, "Found Good\n");
*/
...
//                              report (severity_error, "FP %x : PFP %x\n",FrameParameters,ParsedFrameParameters);
#ifdef DUMP_HEADERS    
				++vopCount;
#endif
				if (DivXVersion == 311)
...
//                      report (severity_error,"%x END_OF_START_CODE_LIST\n",END_OF_START_CODE_LIST);
		...
		{
			report( severity_error, "FrameParser_VideoDivx_c::ReadHeaders - Unknown/Unsupported header 0x%02x\n", Code );
...
//      if (ParsedFrameParameters->NewFrameParameters == true)
...
FrameParserStatus_t   FrameParser_VideoDivx_c::CommitFrameForDecode(void)
{
	...
	Mpeg4VopHeader_t*  Vop = &(VFP->VopHeader);
      //report (severity_info,"%s : %d\n",__FUNCTION__,Vop->prediction_type);
	...
	ParsedVideoParameters->Content.Progressive = !Interlaced;//(StreamParameters->VolHeader.interlaced);
	...
	ParsedVideoParameters->InterlacedFrame = Interlaced; //StreamParameters->VolHeader.interlaced;          
/*
	int n = ParsedFrameParameters->DataOffset;
	report (severity_error, "%x %x %x %x %x %x %x %x %x %x\n",
...
//
    ...
    for( Bit = Bits.Get(1); (Bit == 1); Bit = Bits.Get(1) )
	Vop->time_base++;
...
//
    ...
    report( severity_info, "        fcode_forward                     %6d\n", Vop->fcode_forward );
    report( severity_info, "        fcode_backward                    %6d\n", Vop->fcode_backward );
#endif
...
//      report(severity_error,"Entry %d - %d - %d + %d\n",i,ReferenceFrameList.EntryCount,ReferenceFramesNeeded,i);   
    }
...
//                      report(severity_error,"Removed Ref Frame\n");
		}

//              report(severity_error,"Adding Ref Frame Index %d slot %d\n",ParsedFrameParameters->DecodeFrameIndex, ReferenceFrameList.EntryCount);
