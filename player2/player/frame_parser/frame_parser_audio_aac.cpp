                                                                        ...
                                                                        AacFrameParsingPurpose_t Action )
{
    ...
    if ( !useSameStreamMux )
    {
        ...
        *SampleCount = 1024 * (hasSbrExtension?2:1);
        *SamplingFrequency *= (hasSbrExtension?2:1);
    } // !useSameStreamMux
    else
    {
        if ( Action == AAC_GET_SYNCHRO )
        {
            if (AvailableBytes <= 0)
            {
                FRAME_TRACE("AAC LOAS parser: warining, we nmight have uncorrectly synchronized...\n");
            }
            else
            {
...
FrameParserStatus_t Status;
    ...
    Status = ParseFrameHeader( BufferData, &ParsedFrameHeader, BufferLength, AAC_GET_FRAME_PROPERTIES );
