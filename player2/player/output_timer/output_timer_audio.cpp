...
#if 1
report( severity_info, "OutputTimer_Audio_c::FillOutFrameTimingRecord - SampleRate = %d, CountMultiplier %d.%06d\n",
		LastSampleRate,
...
report( severity_info, "Timing  - ORA = %d.%06d - Samples = %4d - DisplayCount = %4d\n", 
    ...
    Configuration.FrameDecodeTime		= max( (unsigned long long)Duration.RoundedLongLongIntegerPart(), Configuration.FrameDecodeTime);
...
unsigned int OutputTimer_Audio_c::LookupMinimumManifestorLatency(unsigned int SamplingFrequency)
{
    ...
    unsigned int numerator =  AudioConfiguration.MinimumManifestorLatencyInSamples * (1000000 >> no_overflow);
    unsigned int denominator = SamplingFrequency >> no_overflow;
