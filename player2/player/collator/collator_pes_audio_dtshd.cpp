...
CollatorStatus_t Collator_PesAudioDtshd_c::DecideCollatorNextStateAndGetLength( unsigned int *FrameLength )
{
  ...
  if (!GotCoreFrameSize)
  {
      ...
      if (Status == CollatorNoError)
      {
          ...
          if (Type == TypeDtshdCore)
          {
              // core synchro found... 
              
              // handles case when CodeOffset <0
              *FrameLength = FrameHeaderLength + CodeOffset;
              // when the synchro is in the header (codeOffset <0) then no need to read any more data:
              // data which has been too much read will be discarded
              // else read the bytes to complete the frame
              CollatorState  = (CodeOffset < 0)?GotCompleteFrame:ReadSubFrame;
          }
          else
          {
              // extension synchro found... 
              *FrameLength   = FrameHeaderLength + CodeOffset;
              // we got the core size, now we'll calculate the entire frame size another way
              CollatorState  = (CodeOffset < 0)?SeekingFrameEnd:ReadSubFrame;
          }
      }
      else
      {
          // synchro not found, but frame is at least the size of the buffer we searched
          *FrameLength = FrameHeaderLength + RemainingElementaryLength;
...
////////////////////////////////////////////////////////////////////////////
///
/// Get the dtshd frame size
///
/// This this method should only be called when the collator has found the 
/// frame size
///
/// \return Collator status code, CollatorNoError indicates success.
///
CollatorStatus_t Collator_PesAudioDtshd_c::GetSpecificFrameLength(unsigned int * FrameLength)
{
  if (!GotCoreFrameSize)
  {
      *FrameLength = 0;
      COLLATOR_ERROR("Error:Call to GetSpecificLength but the core size is not known...\n");
      return CollatorError;
  }
  else
  {
