...
CollatorStatus_t Collator_PesAudioLpcm_c::DecideCollatorNextStateAndGetLength( unsigned int *FrameLength )
{
  ...
  int StuffingBytesLength = NextParsedFrameHeader.PrivateHeaderLength - Configuration.ExtendedHeaderLength;
  ...
  if ( IsFirstPacket )
    {
      ...
      *FrameLength = StuffingBytesLength + NextParsedFrameHeader.FirstAccessUnitPointer + 
	FirstAccessUnitOffset[StreamType] - NextParsedFrameHeader.PrivateHeaderLength;
  ...
  if ( !IsPesPrivateDataAreaValid )
       ...
       IsPesPrivateDataAreaNew )
    {
      ...
      // prevent accumulating anything for this frame, since we must accumulate 
      // the pda first for next frame
      *FrameLength = 0;
  ...
  else
    {
      ...
      // according to the private data area previously parsed, compute the frame size
      if ((StuffingBytesLength > 0) && (RemainingElementaryLength <= NextParsedFrameHeader.AudioFrameSize))
	{
	  // we will proceed into 3 steps if the private data area length is 
	  // bigger than the default one (stuffing bytes of dvd audio):
	  // 1: read elementary data up to the packet end
	  // 2: skip packet stuffing bytes
	  // 3: read rest of elementary data in the next packet
	  RemainingDataLength = NextParsedFrameHeader.AudioFrameSize - RemainingElementaryLength;
	  PesPrivateToSkip    = StuffingBytesLength;
	  *FrameLength        = RemainingElementaryLength;
	}
      else
	{
...
CollatorStatus_t Collator_PesAudioLpcm_c::HandlePesPrivateData(unsigned char *PesPrivateData)
{
  ...
  if (FPStatus != FrameParserNoError)
	...
	{
	  // check for the length of the stuffing bytes in the pda:has it changed versus last frame?
	  // if yes adjust the number of bytes to skip...
	  PesPrivateToSkip += (NextParsedFrameHeader.PrivateHeaderLength - ParsedFrameHeader.PrivateHeaderLength);
...
////////////////////////////////////////////////////////////////////////////
///
/// Reset the VLPCM addition elementary stream state.
///
