...
Manifestor_Base_c::Manifestor_Base_c    (void)
{
    ...
    Configuration.OutputRateSmoothingFramesBetweenReCalculate = 256;
...
//
    ...
    else
    {
        ...
        if( Status != ManifestorNoError )
        {
            report( severity_error, "Manifestor_Base_c::GetDecodeBuffer(%s) - Failed to flesh out request structure.\n", Configuration.ManifestorName );
            return Status;
        }
    ...
    if( Status != ManifestorNoError )
    {
        report( severity_error, "Manifestor_Base_c::GetDecodeBuffer(%s) - Failed to obtain a buffer.\n", Configuration.ManifestorName );
        return Status;
    }
...
Rational_t	SubPartValue1;
	...
	OutputRateSmoothingLastValue			= OutputRateSmoothingBaseValue;
    }
