...
static unsigned long long  TS[COUNT], TONC[COUNT], DM[COUNT];
static unsigned int SA[COUNT];

    TONC[C]    = TimeOfNextCommit;
    TS[C]      = TimeStamp;
    SA[C]       = SamplesAvailable;
    DM[C++]            = DelayInMicroSeconds;

    if( C==COUNT )
    {
        for( C=1; C<COUNT; C++ )
            report( severity_info, "%4d - Vals %016llx %016llx %4d %6lld - Deltas %6d %6lld %6lld\n", C,
                                TS[C], TONC[C], SA[C], DM[C], SA[C]-SA[C-1], TS[C]-TS[C-1], TONC[C]-TONC[C-1] );
