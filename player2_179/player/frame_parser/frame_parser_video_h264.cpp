...
//
    ...
    *Flags	= 0;
    ...
    // If we have a slice header, and it is has first macroblock is zero, then it is a frame partition point.

    if( ((NalUnitType == NALU_TYPE_IDR) ||
	 (NalUnitType == NALU_TYPE_AUX_SLICE) ||
         (NalUnitType == NALU_TYPE_SLICE)) &&
	 ((HeaderBytes[0] & 0x80) != 0) )
...
//
	...
	switch( nal_unit_type )
	{
	    ...
	    case NALU_TYPE_SLICE:
			...
			// Commit for decode if status is ok, and this is not the first slice (IE we haven't already got the frame to decode flag)
