...
#define MAX_REFERENCE_FRAMES_FOR_FORWARD_DECODE         REFERENCE_FRAMES_NEEDED(AVS_PICTURE_CODING_TYPE_B)
        ...
        switch (ExtractStartCodeCode(Code))
        {
            ...
            default:
                ...
                #if 0
                {
                    ...
                    Status                  = ReadSliceHeader(ExtractStartCodeCode(Code), ExtractStartCodeOffset(Code));
                    break;
                }
                #endif
...
FrameParserStatus_t   FrameParser_VideoAvs_c::ReadPictureHeader (unsigned int PictureStartCode)
{
    ...
    FRAME_DEBUG("%s - %d, %d, %d, %d, %d\n", __FUNCTION__, Header->picture_distance, Header->tr, Header->imgtr_next_P,  Header->imgtr_last_P, Header->imgtr_last_prev_P);
...
FrameParserStatus_t   FrameParser_VideoAvs_c::ReadSliceHeader (unsigned int StartCodeIndex)
{
    ...
    Slice->slice_start_code             = SliceStartCode;
...
FrameParserStatus_t   FrameParser_VideoAvs_c::CalculateFieldOffsets (unsigned int FirstSliceCodeIndex)
{
    ...
    for (unsigned int i=FirstSliceCodeIndex+1; i<StartCodeList->NumberOfStartCodes; i++)
    {
        Code                            = StartCodeList->StartCodes[i];
        SliceCode                       = ExtractStartCodeCode(Code);
        FRAME_TRACE("%s top slice %d at %d\n", __FUNCTION__, ExtractStartCodeCode(Code), ExtractStartCodeOffset(Code));

        if (SliceCode >= MacroblockRows)
        {
...
FrameParserStatus_t   FrameParser_VideoAvs_c::CommitFrameForDecode (void)
{
    ...
    ParsedFrameParameters->NewStreamParameters                  = StreamParameters->UpdatedSinceLastFrame;
