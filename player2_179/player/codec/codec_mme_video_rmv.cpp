...
CodecStatus_t   Codec_MmeVideoRmv_c::CheckCodecReturnParameters( CodecBaseDecodeContext_t *Context )
{
  ...
  if ( AdditionalInfo_p != NULL) {
    ...
    if( AdditionalInfo_p->ErrorCode != RV89DEC_NO_ERROR ) {
      ...
      {
        case RV89DEC_TIMEOUT_ERROR:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_TIMEOUT_ERROR             %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
        case RV89DEC_RUN_TIME_INVALID_PARAMS:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_RUN_TIME_INVALID_PARAMS   %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
        case RV89DEC_FEATURE_NOT_IMPLEMENTED:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_FEATURE_NOT_IMPLEMENTED   %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
        case RV89DEC_MEMORY_UNDERFLOW_ERROR:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_MEMORY_UNDERFLOW_ERROR    %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
        case RV89DEC_MEMORY_TRANSLATION_ERROR:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_MEMORY_TRANSLATION_ERROR  %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
        case RV89DEC_TASK_CREATION_ERROR:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_TASK_CREATION_ERROR       %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
	case RV89DEC_UNKNOWN_ERROR:
	{
	  report( severity_info,  "Codec_MmeVideoRmv_c::CheckCodecReturnParameters - RV89DEC_UNKNOWN_ERROR             %x \n" , AdditionalInfo_p->ErrorCode );
	  break;
	}
      }
    }
