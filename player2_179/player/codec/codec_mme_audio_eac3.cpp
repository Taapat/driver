...
//
    ...
    if (isFwEac3Capable)
    {
        ...
        // bit 3: dithermode (0 -> DRC, 1 -> DEC71) (must be 0 for correct transcoding)
	// bit 5: disable LFE downmix when set
		Config.Config[DDP_OUTPUT_SETTING] =
			0x2 |                  // upsampling is never done
			(ACC_MME_TRUE << 2) |  // little endian transcoded output
			(ACC_MME_FALSE << 3) |  // DRC dither mode
			((!EnableLfeDMix) << 5);
