...
static void SetGlobalTransformParameters(avr_v4l2_audio_handle_t *AudioContext,
...
/*{{{  store_xxx_attribute*/
ssize_t store_decode_errors(struct class_device *class_dev, const char *buf, size_t count)
{
...
ssize_t show_input_format(struct class_device *class_dev, char *buf)
{
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    ssize_t attr_size;
    int is_master_audio;
    ...
    attr_size = sprintf(buf, "%s%s\n", TranslatePcToEncoding(AudioContext->LLDecoderStatus.CurrentSpdifStatus.PC),
			(is_master_audio ? " MA" : ""));
...
/* creation of show_xx methods */
ssize_t show_decode_errors(struct class_device *class_dev, char *buf)
{
...
/* creation of show_xx methods */
ssize_t show_number_channels(struct class_device *class_dev, char *buf)
{
    // get the private context 
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    enum eAccAcMode AudioMode;
...
#if SPDIFIN_API_VERSION >= 0x090623
    report_audio(&AudioContext->LLDecoderStatus.CurrentSpdifStatus,  &front, &back, &lfe, &misc);

    nchan = front + back + lfe + misc;
    if (nchan <= 0 || nchan > 8) {
	DVB_ERROR("Bad firmware channel assignment report (%d/%d.%d) - ignoring\n", front, back+misc, lfe);
    ...
    count_speakers(ChannelAssignment, &front, &back, &lfe, &misc);
#endif  
    DVB_TRACE("%s (0x%02x) -> %d/%d.%d\n", LookupAudioMode(AudioMode), AudioMode, front, back+misc, lfe);
    return sprintf(buf, "%d/%d.%d\n", front, back+misc, lfe);
}

/* creation of show_xx methods */
ssize_t show_sample_frequency(struct class_device *class_dev, char *buf)
{
    // get the private context 
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    ssize_t attr_size;
...
static bool is_supported(avr_v4l2_audio_handle_t *AudioContext)
{
    ...
    switch (AudioContext->LLDecoderStatus.CurrentSpdifStatus.PC)
    {
	    ...
	    return Capability->DecoderCapabilityExtFlags[0] & 0x8; // ACC_SPDIFIN_DD
        ...
        case SPDIFIN_IEC60958_DTS3_16:
            ...
            return Capability->DecoderCapabilityExtFlags[0] & 0x10; // ACC_SPDIFIN_DTS

        case SPDIFIN_MP2AAC:
	    return Capability->DecoderCapabilityExtFlags[0] & 0x20; // ACC_SPDIFIN_MPG to be renamed ACC_SPDIFIN_AAC
...
/* creation of show_xx methods */
ssize_t show_supported_input_format(struct class_device *class_dev, char *buf)
{
    // get the private context 
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    bool IsInconsistant;
...
/* creation of show_xx methods */
ssize_t show_supported_input_format_in_game_mode(struct class_device *class_dev, char *buf)
{
    // get the private context 
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    bool IsSupported;
...
/* creation of show_xx methods */
ssize_t show_number_of_samples_processed(struct class_device *class_dev, char *buf)
{
    // get the private context 
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    ssize_t attr_size;
...
ssize_t store_emergency_mute(struct class_device *class_dev, const char *buf, size_t count)
{
	avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
...
ssize_t show_emergency_mute(struct class_device *class_dev, char *buf)
{
    // get the private context 
    avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
    char * str_p;
...
ssize_t store_post_mortem(struct class_device *class_dev, const char *buf, size_t count)
{
	avr_v4l2_audio_handle_t *AudioContext = (avr_v4l2_audio_handle_t *) class_dev->class_data;
	ssize_t res;
...
static int CreateSysFsStreamAndAttributes( avr_v4l2_audio_handle_t *AudioContext )
{
    ...
    result = PlaybackGetPlayerEnvironment (AudioContext->DeviceContext->Playback,
                                           &playerplayback);
    if (result < 0) 
    {
...
int AvrAudioLLThread (void *data)
{
	...
	else
	{
...
#if DRV_MULTICOM_AUDIO_DECODER_VERSION >= 0x090128
	    NewMute = LLDecoderStatus->CurrentMuteStatus = PrimaryLimiterStatus->EmergencyMute.bits;
#endif
            down(&AudioContext->DecoderStatusSemaphore);
	    ...
	    CodecChange = (OldState != NewState) || (OldPC != NewPC);
	    ...
	    TopologyChange = (LLDecoderStatus->CurrentDecoderAudioMode != DecoderStatus->DecAudioMode);
            ...
            if ( CodecChange || MuteChange || FrequencyChange || TopologyChange )
            {
                ...
                DVB_TRACE("LL thread: Firmware /%s%s%s%s change: /%s-%s/%d/%s/\n",
			  (CodecChange ? "Codec/" : ""), (MuteChange ? "Mute/" : ""),
                          (FrequencyChange ? "Frequency/" : ""), (TopologyChange ? "Topology/": ""),
			  TranslatePcToEncoding(NewPC), LookupSpdifInState(NewState),
...
#ifndef DISABLE_ZERO_DECODER_CAPABILITY_EXT_WORKAROUND
    if (0 == LLInfo->DecoderCapabilityExtFlags[0]) {
	DVB_ERROR( "Firmware reports utter incapability - ignoring\n");
	LLInfo->DecoderCapabilityExtFlags[0] = 0xffffffff;
    }
#endif
                                          ...
                                          MME_LowlatencySpecializedGlobalParams_t * GlobalParams)
{
	...
	if (mixer_settings->drc_enable) {
        ...
        for (out_idx = 0; out_idx < ActualOutputCards; out_idx ++)
        {
            ...
            result = ksnd_pcm_open(&AudioContext->SoundcardHandle[out_idx], CardId, DevId, SND_PCM_STREAM_PLAYBACK);
...
#else // DRV_MULTICOM_AUDIO_DECODER_VERSION >= 0x090128
...
//XXX( ACC_MODE32_LFE,       	L_R,		CNTR_LFE1,	LSUR_RSUR,	LHIGHSIDE_RHIGHSIDE ), No enum
					   ...
					   int dev_num )
{
	...
	if (ACC_MODE_1p1 == InputMode) {
		if (AudioContext->ChannelSelect == V4L2_AVR_AUDIO_CHANNEL_SELECT_STEREO)
			InputMode = ACC_MODE20;
		else
			InputMode = ACC_MODE10;
	}
					 ...
					 int dev_num, OutputEncoding_t OutputEncoding )
{
    ...
    if( OutputEncoding == OUTPUT_FATPIPE )
    {
        ...
        // TODO: inform the firmware that it should manage the metadata internally
        // i.e. get them from the decoder by itself
...
{
#define EI(in, out) { in, out }, { in ## _LFE, out }
#define EL(in, out) { in, out }, { in ## _LFE, out ## _LFE }
#define EX(in, out) { in, out } 
	EX(ACC_MODE10, ACC_MODE_ID),
	EI(ACC_MODE20, ACC_MODE_ID),
	EI(ACC_MODE30, ACC_MODE_ID),
	//EL(ACC_MODE40, ACC_MODE30),
	//EL(ACC_MODE50, ACC_MODE30),
	EI(ACC_MODE21, ACC_MODE_ID),
	EI(ACC_MODE31, ACC_MODE_ID),
	//EL(ACC_MODE41, ACC_MODE31),
	//EL(ACC_MODE51, ACC_MODE31),
	EI(ACC_MODE22, ACC_MODE_ID),
	EI(ACC_MODE32, ACC_MODE_ID),
	...
	EI(ACC_MODE33, ACC_MODE_ID),
	EL(ACC_MODE24, ACC_MODE34),
	EI(ACC_MODE34, ACC_MODE_ID),
...
///
enum eAccAcMode LookupFatPipeOutputMode( enum eAccAcMode InputMode )
{
    int i;
                                         ...
                                         bool DeployEmergencyMute)
{
	...
	CMC->Config[CMC_DUAL_MODE] =
	    ...
	    CMC->Config[CMC_OUTMODE_MAIN] =
		    LookupFatPipeOutputMode(AudioContext->LLDecoderStatus.CurrentDecoderAudioMode);
        }
...
void FillOutLLSetGlobalCommand(avr_v4l2_audio_handle_t *AudioContext)
{
    ...
    // update transformer global parameters
    SetGlobalTransformParameters(AudioContext, &AudioContext->SetGlobalParams);
