...
static int linuxdvb_v4l2_capture_thread( struct linuxdvb_v4l2 *ldvb )
{
	...
	while (ldvb->capture && ldvb->capture->running) {
		int ret;
		stm_display_buffer_t *ptr;

		ret = wait_event_interruptible_timeout (g_ManifestorLastWaitQueue,
...

		{
			...
			srcrect.right  = buffer.src.Rect.width;
			srcrect.bottom = buffer.src.Rect.height;
...
static int linuxdvb_close(struct stm_v4l2_handles *handle, enum _stm_v4l2_driver_type type, struct file *file)
{
	...
	if (ldvb->capture) {
		...
		ldvb->capture->running = 0;
                ...
                wait_event (buffer_blitted, ldvb->capture->complete == -1);
