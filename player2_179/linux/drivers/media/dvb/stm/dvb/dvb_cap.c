				   ...
				   struct stm_v4l2_handles *handle)
{
	...
	{
		ret = PlaybackCreate (&shared_context->cap_device_context.Playback);
		if (ret < 0)
		{
	 ...
	 * drift appart.
	 */
	ret = PlaybackSetOption (shared_context->cap_device_context.Playback,
	...
	ret = PlaybackGetPlayerEnvironment (shared_context->cap_device_context.Playback,
                                            &playerplayback);
	    if (ret < 0) {
				  ...
				  struct stm_v4l2_handles *handle)
{
	...
	ret = PlaybackDelete(shared_context->cap_device_context.Playback);
	if (ret < 0) {
