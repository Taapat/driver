...
inline int spark_avs_standby(int type)
{
	if ((type<0) || (type>1))
	{
		return -EINVAL;
...
int spark_avs_command(unsigned int cmd, void *arg )
{
	...
	if (cmd & AVSIOSET)
	{
		...
		{
			case AVSIOSVOL:
				return spark_avs_set_volume(val);
			case AVSIOSMUTE:
				return spark_avs_set_mute(val);
			case AVSIOSTANDBY:
				return spark_avs_standby(val);
			default:
				return -EINVAL;
		}
	}
	else if (cmd & AVSIOGET)
	{
		...
		{
			case AVSIOGVOL:
				val = spark_avs_get_volume();
				break;
			case AVSIOGMUTE:
				val = spark_avs_get_mute();
				break;
			default:
				return -EINVAL;
		}
	...
	else
	{
		...
		{
		case SAAIOSMODE:
           		 return spark_avs_set_mode(val);
 	        case SAAIOSENC:
        		 return spark_avs_set_encoder(val);
		case SAAIOSWSS:
			return spark_avs_set_wss(val);
		case SAAIOSSRCSEL:
        		return spark_avs_src_sel(val);
		default:
