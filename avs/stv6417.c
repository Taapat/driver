...
unsigned char regs[STV6417_MAX_REGS + 1]; /* range 0x01 to 0x10 */

unsigned char backup_regs[STV6417_MAX_REGS + 1];
...
unsigned char vcr_value;

static int stv6417_s_old_src;

void set_bits(int regIndex, unsigned char value, int start_bit, int nr_bits)
{
    regs[regIndex] = (regs[regIndex] & (~(((1 << nr_bits) - 1) << start_bit))) | (value << start_bit);
}

unsigned char get_bits(int regIndex, int start_bit, int nr_bits)
{
...
inline int stv6417_set_mute( struct i2c_client *client, int type )
{
	...
	if (type == AVS_MUTE) 
	{
		if (tv_value == 0xff)
		{
		   tv_value = get_bits(cReg1, 0, 2); 
		   vcr_value = get_bits(cReg1, 3, 2); 

	           set_bits(cReg1, 0, 0, 2); /* tv cinch mute */
	           set_bits(cReg1, 0, 3, 2); /* vcr mute */
		}
	}
	else /* unmute with old values */
	{
		if (tv_value != 0xff)
		{
	           set_bits(cReg1, tv_value, 0, 2);
	           set_bits(cReg1, vcr_value, 3, 2);
		...
		case 1:	// rgb
			if (type<0 || type>2)
			{
				return -EINVAL;
	                ...
	                set_bits(cReg3, type, 2, 2);
			break;
		case 2: // tv
	                set_bits(cReg2, type, 0, 3);
			break;
		default:
			return -EINVAL;
	}
 ...
 */
	...
	if (vol == SAA_WSS_43F)
	{
           set_bits(cReg6, 3, 2, 4);   
	}
	else if (vol == SAA_WSS_169F)
	{
           set_bits(cReg6, 2, 2, 4);   
	}
	else if (vol == SAA_WSS_OFF)
	{
           set_bits(cReg6, 0, 2, 4);   
	}
	else
	{
		return  -EINVAL;
...
inline int stv6417_get_vsw( int sw )
{
	...
	{
		case 0:
			return get_bits(cReg2, 4, 3);
			break;
		case 1:
			return get_bits(cReg3, 2, 2);
			break;
		case 2:
			return get_bits(cReg2, 0, 3);
			break;
		default:
			return -EINVAL;
	}
...
inline int stv6417_get_asw( int sw )
{
	...
	switch(sw)
	{
		case 0:
			...
			if ( vcr_value == 0xff )
				return get_bits(cReg1, 3, 2);
			else
				return vcr_value;
		case 1:
		case 2:
			if ( tv_value == 0xff )
				return get_bits(cReg1, 0, 2);
			else
				return tv_value;
			break;
		default:
...
int stv6417_set_mode( struct i2c_client *client, int vol )
{
	...
	if (vol == SAA_MODE_RGB)
	{
		if(get_bits(cReg2, 0, 3) == 4) // scart selected
			stv6417_s_old_src = 1;
		else
			set_bits(cReg2, 1, 0, 3);
                set_bits(cReg3, 1, 0, 2);    /* fast blanking */   
	}
	else if (vol == SAA_MODE_FBAS)
	{
		if(get_bits(cReg2, 0, 3) == 4) // scart selected
			...
			set_bits(cReg2, 1, 0, 3);
                set_bits(cReg3, 0, 0, 2);   /* fast blanking */
	}
...
int stv6417_src_sel( struct i2c_client *client, int src )
{
	...
	if (src == SAA_SRC_ENC)
	{
	   set_bits(cReg2, stv6417_s_old_src, 0, 3); /* t_vsc */
	   set_bits(cReg2, stv6417_s_old_src, 4, 3); /* v_vsc */
	   set_bits(cReg1, 1, 0, 2); /* tc_asc */
	   set_bits(cReg1, 1, 3, 2); /* v_asc */
	}
	else if(src == SAA_SRC_SCART)
	{
	   stv6417_s_old_src = get_bits(cReg2, 0, 3);
	   set_bits(cReg2, 4, 0, 3); 
	   set_bits(cReg2, 0, 4, 3);
	   set_bits(cReg1, 2, 0, 2);
	   set_bits(cReg1, 0, 3, 2);
  	}
  	else
	{
		return  -EINVAL;
...
inline int stv6417_standby( struct i2c_client *client, int type )
{
	...
	if (type==1) 
	{
		if (t_stnby == 0)
		{
			...
			// Full Stop mode
                        set_bits(cReg8, 1, 0, 1); /* enc in */
                        set_bits(cReg8, 1, 1, 1); /* vcr in */
                        set_bits(cReg8, 1, 2, 1); /* tv  in */
                        set_bits(cReg8, 1, 3, 1); /* vcr out */
                        set_bits(cReg8, 1, 4, 1); /* tv  out */
                        set_bits(cReg8, 1, 5, 1); /* rf  out */
                        set_bits(cReg8, 1, 6, 1);
                        set_bits(cReg8, 1, 7, 1);
